...
|---|---|---|---|
| `body` | string | One of `body`/`text` | Alert message body |
| `text` | string | One of `body`/`text` | Alias for `body`, supported for dsh-notification |
| `title` | string | | Notification title |
| `summary` | string | | Alias for `title`, supported for dsh-notification |
| `level` | string | | Severity level (e.g. `critical`, `warning`) |
| `url` | string | | Click-through URL |
| `group` | string | | Alert grouping label |

The `/alert` endpoint accepts dsh-notification payloads by mapping `text` to `body` and `summary` to `title`; additional fields such as `ts` are ignored.
